...
static const char *getPropertyType(objc_property_t property)
{
    ...
    char buffer[1 + strlen(attributes)], result[1 + strlen(attributes)];
    ...
    while ((attribute = strsep(&state, ",")) != NULL)
    {
        if (attribute[0] == 'T' && attribute[1] != '@') {
            strncpy(buffer, attribute + 1, strlen(attribute) - 1);
            return (const char *)[[NSData dataWithBytes:buffer length:strlen(buffer) + 1] bytes];
//            return (const char *)[[NSData dataWithBytes:(attribute + 1) length:strlen(attribute) - 1] bytes];
        } else if (attribute[0] == 'T' && attribute[1] == '@' && strlen(attribute) == 2) {
            return "id";
        } else if (attribute[0] == 'T' && attribute[1] == '@') {
            strncpy(result, attribute + 3, strlen(attribute) - 4);
            return (const char *)[[NSData dataWithBytes:result length:strlen(result) + 1] bytes];
